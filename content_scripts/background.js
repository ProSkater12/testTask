/*Взято из ассетов. Инструмент для создание хеша MD5. (потому что в js
своего хеширования)*/
...
function User(mail, pass) {
    this.mail = mail;
    this.password = pass;
    this.playlists = [new playlist('DIY 3D Printers'), new playlist('MTV Base'), new playlist('Concept Cars')];
}
...
var active_user = new User("erfeffe@bk.ru", MD5("fvijsjfis2321"));

//слушатели
chrome.extension.onMessage.addListener(function(request, sender, f_callback){
	if(request=='setUser'){ //проверяется, от того ли окна и скрипта отправлено
...
function add_URL(id){
  chrome.tabs.query({active: true},function(tabs){
    ...
    //Добавляем ссылку в плейлист пользователя
    active_user.playlists[id].objects.unshift(tabs[0].url);
    console.log(active_user.playlists[id].objects);
  });
  /*alert(
...
var parent = chrome.contextMenus.create({"title": "TV by Playlist (username)"});

var childs = [];//Плейлисты
if(active_user){
